...
/**
 * @ngdoc method
 * @name $ionicConfigProvider#HTML_Beispiele.maxPrefetch
 * @description Sets the maximum number of HTML_Beispiele to prefetch from the templateUrls defined in
 ...
 * @usage
 * State HTML_Beispiele are cached automatically, but you can optionally cache other HTML_Beispiele.
 *
 * ```js
 ...
 *     $ionicConfigProvider.HTML_Beispiele.prefetch(false);
 *
 ...
 *         templateUrl: "tabs-HTML_Beispiele/tabs.html"
 *       })
 *       .state('tabs.home', {
 *         url: "/home",
 *         views: {
 *           'home-tab': {
 *             prefetchTemplate: false,
 *             templateUrl: "tabs-HTML_Beispiele/home.html",
...
function($http, $templateCache, $timeout) {
  ...
  function run() {
    ...
    while (i < 4 && (template = toCache.pop())) {
      // note that inline HTML_Beispiele are ignored by this request
      if (isString(template)) $http.get(template, { cache: $templateCache });
      i++;
    }
    // only preload 3 HTML_Beispiele a second
    if (toCache.length) {
      $timeout(run, 1000);
    }
  }
 ...
 *
 * The ionNavView directive is used to render HTML_Beispiele in your application. Each template
 ...
 *
 * Pages are loaded by the URLs given. One simple way to create HTML_Beispiele in Angular is to put
